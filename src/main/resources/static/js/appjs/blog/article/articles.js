								...
								},
								{
									...
									formatter : function(value, row, index) {
										...
										var d = '<a class="btn btn-warning btn-sm '+s_remove_h+'" href="#" title="删除"  mce_href="#" onclick="remove(\''
												...
												+ '\')">删除</a> ';
										if (row.status == '0') {
											var f = '<a class="btn btn-success btn-sm '+s_status_h+'" href="#" title="发布"  mce_href="#" onclick="updateStatus(\''
												+ row.id + '\',\'1\')">发布</a> ';
...
function updateStatus(id,status) {
	...
	if (status=="0"){
		layer.confirm('确定要隐藏该文章吗？隐藏之后将对用户不可见', {
			btn : [ '确定', '取消' ]
		},function() {
			$.ajax({
				url : prefix + "/status",
		...
		layer.confirm('确定要发布该文章吗？发布之后将直接显示在博客页面', {
			btn : [ '确定', '取消' ]
		},function() {
			$.ajax({
				url : prefix + "/status",
